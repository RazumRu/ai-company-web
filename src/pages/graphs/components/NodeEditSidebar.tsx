...
export const NodeEditSidebar = ({
  ...
  const applyReasoningEntries = (
    entries: ReasoningChunkEntry[],
    context: { externalThreadId?: string; runId?: string },
  ) => {
    if (!entries.length || !selectedThreadId || !node?.id) return;
  ...
  useWebSocketEvent(
    ...
    (notification) => {
      ...
      if (!externalThreadId && eventThreadId) {
        setExternalThreadId(eventThreadId);
      }
  ...
  useEffect(() => {
    const reasoningChunks =
      compiledNode?.additionalNodeMetadata?.reasoningChunks;
    ...
    const metadataRunId =
      typeof compiledNode?.metadata?.runId === 'string'
        ? compiledNode.metadata.runId
        : undefined;

    const container = narrowReasoningContainer(reasoningChunks, [
      metadataThreadId ?? externalThreadId,
      selectedThreadId,
      metadataRunId,
    ]);

    const entries = extractReasoningEntries(container, {
      threadId: metadataThreadId ?? externalThreadId,
      runId: metadataRunId,
    });
    ...
    applyReasoningEntries(entries, {
      externalThreadId: metadataThreadId ?? externalThreadId,
      runId:
        metadataRunId ??
