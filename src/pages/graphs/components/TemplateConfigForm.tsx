    ...
    <Form
      ...
      templates={{
        ...
        FieldTemplate: (
          ...
          const schemaType = (props.schema as { type?: unknown } | undefined)
            ?.type;
          ...
          const showLabel =
            Boolean(props.label) &&
            (props.displayLabel !== false || isBooleanField);
          const descriptionText =
            typeof props.rawDescription === 'string'
          ...
          return (
            <div style={{ marginBottom: 15 }}>
              ...
              <div style={{ marginTop: showLabel ? 4 : 0 }}>
                {props.children}
              </div>
      ...
      widgets={{
        // Use AntD Select for enums so "Reasoning" looks the same as "Model"
        SelectWidget: (props: WidgetProps<string, RJSFSchema, FormContext>) => {
          const enumOptions = (
            (
              props.options as {
          ...
          const placeholder =
            props.placeholder ??
            (props.label
              ? `Select ${String(props.label).toLowerCase()}`
              : 'Select');
        ...
        NumberWidget: (props: WidgetProps<number, RJSFSchema, FormContext>) => {
        ...
        UpDownWidget: (props: WidgetProps<number, RJSFSchema, FormContext>) => (
          <div onWheelCapture={handleNumberWheel}>
            <InputNumber
              value={
        ...
        // Use toggle switch instead of checkbox for boolean fields
        CheckboxWidget: (
        ...
        litellmModelsSelect: (
          ...
          const ctx = getFormContext(props);
          const options = (ctx.liteLlmModels ?? []).map(
            (model: LiteLlmModelDto) => ({
              label: model.ownedBy
      ...
      fields={{
        ...
        keyValueObject: (
          ...
          const pairs: KeyValuePair[] = Object.entries(record).map(
            ([k, v]) => ({
              key: k,
