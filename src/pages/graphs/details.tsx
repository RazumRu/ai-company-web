...
import { useGraphThreads } from './hooks/useGraphThreads';
...
export const GraphPage = () => {
  ...
  // Use a ref for fetchCompiledNodes so handleThreadChange can be stable
  // and passed to useGraphThreads before fetchCompiledNodes is defined.
  const fetchCompiledNodesRef = useRef<
    (options?: {
      threadId?: string;
  ...
  // Use threadsRef to avoid recreating fetchCompiledNodes on every thread update
  ...
  const fetchCompiledNodes = useCallback(
    async (options?: {
      ...
      // Read threads from ref to avoid dependency on threads array
      const selectedThread = threadsRef.current.find(
        (t) => t.id === threadIdToUse,
      );
      ...
      try {
        ...
        setCompiledNodesMap((prevMap) => {
          ...
          // Check if any node data changed (field-level comparison to avoid JSON.stringify)
  ...
  useEffect(() => {
    fetchCompiledNodesRef.current = fetchCompiledNodes;
  }, [fetchCompiledNodes]);
  ...
  // Cache the selected thread lookup to avoid repeated linear scans in JSX
  ...
  return (
    <Layout style={{ overflow: 'hidden' }}>
      <GraphPageHeader
        ...
        onThreadPopoverChange={setThreadPopoverVisible}
        onThreadSelect={handleThreadSelect}
        onDeleteThread={handleDeleteThread}
        onStopThread={handleStopThread}
        onOpenThreadChat={handleOpenThreadChat}
      ...
      <Layout
        ...
        <NodeEditSidebar
          ...
          selectedThreadLastRunId={selectedThread?.lastRunId}
          compiledNode={
      ...
      <TriggerModal
        ...
        selectedThreadSource={selectedThread?.source ?? null}
      />
      ...
      <ThreadChatModal
        ...
        onClose={handleCloseThreadChat}
        triggerNodes={triggerNodesForGraph}
        nodeDisplayNames={nodeDisplayNames}
        defaultNewMessageMode={defaultNewMessageMode}
        messages={chatThread ? messages[chatThread.id]?.['all'] || [] : []}
        messagesLoading={
          chatThread ? getMessageMeta(chatThread.id, undefined).loading : false
        }
        hasMoreMessages={
          chatThread ? getMessageMeta(chatThread.id, undefined).hasMore : false
        }
        loadingMoreMessages={
          chatThread
        ...
        pendingMessages={
          chatThread ? pendingMessages[chatThread.id]?.['all'] || [] : []
        }
        externalThreadId={
          chatThread ? externalThreadIds[chatThread.id] : undefined
        }
        onLoadMoreMessages={() =>
          chatThread
