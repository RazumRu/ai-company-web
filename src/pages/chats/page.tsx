...
export const ChatsPage = () => {
  ...
  const [threadStatusTab, setThreadStatusTab] =
    useState<ThreadStatusTabKey>('open');
  ...
  const filteredThreads = useMemo(() => {
    ...
    if (threadStatusTab === 'resolved') {
      return threads.filter((thread) => isResolvedThreadStatus(thread.status));
    }
    return threads.filter((thread) => !isResolvedThreadStatus(thread.status));
  }, [threadStatusTab, threads]);
  ...
  useEffect(() => {
    ...
    if (threadStatusTab === 'all') return;
    const selectedIsResolved = isResolvedThreadStatus(selected.status);
    const shouldBeVisible =
      threadStatusTab === 'resolved' ? selectedIsResolved : !selectedIsResolved;
    if (!shouldBeVisible) {
      setSelectedThreadId(undefined);
    }
  }, [
